		...
		<form on:submit={handleSubmit} method="post" action="?/vote">
			...
			<div class="flex flex-row items-center">
				...
				{#if poll.status === 'draft'}
					<span
						class="ml-3 text-xs px-3 rounded-xl bg-secondary-700 py-0.5 text-surface-50 select-none"
						>Draft</span
					>
				{/if}
			</div>
			...
			{:else if poll.type === 'image'}
				...
				>
					{#each poll.answers as answer (answer.id)}
						<div class="flex items-center gap-2 px-4 py-2 group overflow-hidden bg-surface-800">
							<input
							...
							>
								<img
									src={answer.image}
									alt={answer.label}
									class="max-w-auto h-[200px] w-auto transition group-hover:scale-105 object-contain"
								/>
								{#if answer.label}
									<p class="self-start mt-1 pl-5 text-surface-300 font-medium">
										{answer.label}
									</p>
								{/if}
							</label>
